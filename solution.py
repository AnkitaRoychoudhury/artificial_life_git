...
class SOLUTION:
    ...
    def Create_Body(self):
        ...
        # all joints should have motors and all neuron pairs must have synapses

        # first check to make sure you're getting placement right
        
        # will always start with a torso
        pyrosim.Send_Cube(name = "0", pos=[0, 0, 1] , size=[width, length, height])
        pyrosim.Send_Joint( name = "0_1" , parent= "0" , child = "1" , type = "revolute", position = [0,-0.5,1], jointAxis = '1 0 0')
        pyrosim.Send_Cube(name = "1", pos=[0,-0.5,0] , size=[0.2, 1, 0.2])
        pyrosim.Send_Joint( name = "0_2" , parent= "0" , child = "2" , type = "revolute", position = [0,0.5,1], jointAxis = '1 0 0')
        pyrosim.Send_Cube(name = "2", pos=[0, 0.5, 0] , size=[0.2, 1, 0.2])
        
        # pyrosim.Send_Cube(name = "0", pos=[0,0,0.5] , size=[width, length, height])

        # pyrosim.Send_Cube(name = "1", pos=[0,0,0.5] , size=[width, length , height])
        # pyrosim.Send_Joint( name = "0_1" , parent= "0" , child = "1" , type = "revolute", position = [0,0,1], jointAxis = '0 0 1')

        
        # pyrosim.Send_Cube(name = "2", pos=[0,0,0.5] , size=[width, length, height])
        # pyrosim.Send_Joint( name = "1_2" , parent= "1" , child = "2" , type = "revolute", position = [0,0,1], jointAxis = '0 0 1')
#
    ...
    def Create_Brain(self):
        ...
        pyrosim.Send_Motor_Neuron(name=3, jointName = "0_1"),
        pyrosim.Send_Motor_Neuron(name=4, jointName = "1_2")
